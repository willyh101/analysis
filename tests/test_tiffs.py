...
@pytest.fixture
def test_file_fails() -> str:
    return '/mnt/scratch/masato/FOV1.tif'

@pytest.fixture(params=['scanimage', 'tifffile'])
...
def test_default_reader_is_scanimage(good_file):
    tif = SItiffCore(good_file)
    assert isinstance(tif._reader, SItiffDataReader)
    assert tif._reader.reader_backend == 'scanimage'
    
def test_backend_failover_to_tifffile(test_file_fails):
    tif = SItiffCore(test_file_fails, backend='scanimage')
    assert isinstance(tif._reader, TiffFileDataReader)
    assert tif._reader.reader_backend == 'tifffile'
    
def test_backend_not_supported(test_file_fails):
    with pytest.raises(ValueError):
        tif = SItiffCore(test_file_fails, backend='not_supported')